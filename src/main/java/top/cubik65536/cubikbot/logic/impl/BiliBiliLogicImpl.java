...
import top.cubik65536.cubikbot.pojo.ResultStatus;
...
public class BiliBiliLogicImpl implements BiliBiliLogic {
    ...
    private Result<List<BiliBiliPojo>> getDynamicById(String id, String offsetId) throws IOException {
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            ...
            if (extraJsonObject != null && Integer.valueOf(1).equals(extraJsonObject.getInteger("is_space_top")))
                continue;
    ...
    public Result<BiliBiliEntity> loginByQr2(String url) throws IOException {
        ...
        if (!status) {
            switch (jsonObject.getInteger("data")) {
                case -2:
                    return Result.failure("您的二维码已过期！！", null);
                case -4:
                    return Result.failure(ResultStatus.QRCODE_NOT_SCANNED);
                case -5:
                    return Result.failure(ResultStatus.QRCODE_IS_SCANNED);
                default:
